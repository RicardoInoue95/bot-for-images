...
# Função para iniciar ou parar a busca por imagens
def alternar_busca():
...
def iniciar_busca():
    ...
    threading.Thread(target=buscar_imagens).start()
...
# Botão para iniciar/parar a busca por imagens
botao_iniciar = tk.Button(janela, text="Iniciar/Parar busca por imagens", command=alternar_busca)
botao_iniciar.pack()
...
nome_arquivo_especial_label.pack(side="left", padx=5)

# Adicionar atalho de teclado para iniciar/parar a busca com a tecla "End"
keyboard.add_hotkey('End', alternar_busca)
